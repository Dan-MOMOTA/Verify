...
class tc_spi_rd extends uvm_test;
...
function void tc_spi_rd::build_phase(uvm_phase phase);
    ...
    //                                        "v_sqr.main_phase",
    //                                        "default_sequence",
    //                                        spi_rd_sequence::get_type()
    //                                       );
endfunction:build_phase

function void tc_spi_rd::connect_phase(uvm_phase phase);
    super.connect_phase(phase);
    ral_model.default_map.set_sequencer(env.apb_agt.apb_sqr,reg_adpt);
    ral_model.default_map.set_auto_predict(1);
endfunction:connect_phase

task tc_spi_rd::main_phase(uvm_phase phase);
    //Method No.2
    spi_wr_sequence seq;
    super.main_phase(phase);
    
    v_sqr.apb_sqr = env.apb_agt.apb_sqr;
    v_sqr.spi_sqr = env.spi_agt.spi_sqr;

    seq = spi_rd_sequence::type_id::create("seq");
    seq.starting_phase = phase;
    seq.start(this.v_sqr);
    phase.phase_done.set_drain_time(this,plus::plus_main_phase_drain_time);
endtask:main_phase

function void tc_spi_rd::report_phase(uvm_phase phase);
