...
function void tc_sanity::connect_phase(uvm_phase phase);
    `uvm_info(get_type_name(),$sformatf("connect_phase() START!"), UVM_MEDIUM)
    super.connect_phase(phase);
    //1.Method No.1
    uvm_config_db#(uvm_object_wrapper)::set(
                                            uvm_root::get(),
                                            $sformatf("%s.main_phase",this.env.gpu_agt.gpu_sqr.get_full_name()),
...
task tc_sanity::main_phase(uvm_phase phase);
    ...
    `uvm_info(get_type_name(),$sformatf("main_phase() START!"), UVM_MEDIUM)
    ...
    `uvm_info(get_type_name(),$sformatf("main_phase() DONE !"), UVM_MEDIUM)
endtask:main_phase
