...
module uart_rx(
	...
	output[10:0] rx_fifo_out, // BI, FE, PE and data
		...
		case(rx_state)
			...
			PARITY: begin
				if((enable == 1) && (bit_counter == 4'hf)) begin
					rx_state <= STOP1;
					case(LCR[5:3])
						3'b001: rx_buffer[8] <= ~(filtered_rxd == ~(^rx_buffer)); // PE
						3'b011: rx_buffer[8] <= ~(filtered_rxd == (^rx_buffer));
						3'b101: rx_buffer[8] <= ~filtered_rxd; // Stick 1
						3'b111: rx_buffer[8] <= filtered_rxd; // Stick 0
						default: rx_buffer[8] <= 0;
					endcase
				end
				if(enable == 1) begin
					bit_counter <= bit_counter + 1;
			...
			STOP1: begin
				parity_error <= rx_buffer[8];
				if((enable == 1) && (bit_counter == 4'hf)) begin
					rx_state <= STOP2;
					rx_buffer[9] <= (filtered_rxd == 0); // FE
					rx_buffer[10] <= ({filtered_rxd, rx_buffer} == 0); // Break error
				end
				if(enable == 1) begin
					bit_counter <= bit_counter + 1;
				end
...
property rx_parity;
	@(posedge PCLK)
		$rose(rx_state == STOP1) |=> parity_error == rx_buffer[8];
...
property rx_framing;
	@(posedge PCLK)
		$rose((rx_state == STOP1) && (enable == 1) && (bit_counter == 4'hf)) |=> rx_buffer[9] == ~filtered_rxd;
